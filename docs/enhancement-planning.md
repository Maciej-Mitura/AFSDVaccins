...
**Status:** Phase 21 planning companion; stack rows for rate limiting / cache / Helmet / GraphQL limits updated after Phase 22; runtime i18n updated after Phase 23C.  
**Evidence date:** 2026-07-24 · Phase 23 (23A–23C) i18n complete locally (FRONT-018 implemented with es/zh Default caveat); do not claim a separate 23C CI green push unless on remote  
...
| Runtime i18n                | Optional or extra in original checklist; mandatory for this project’s chosen Tier A final scope         | `vue-i18n` + `packages/i18n-export` + `locales/{nl,en,zh,es}.json` (481 keys) | Implemented          | zh/es mostly Default (EN) export fallback (~415); not verified human copy         | Maintain; improve Sheet columns                  |
...
| External public deployment  | Optional or extra (bonus) in original checklist; mandatory for this project’s chosen Tier A final scope | 24B readiness in repo; manual deploy pending (`docs/deployment.md`)           | Missing (until live) | DEVOPS-007 incomplete until public URL verified                                   | Phase 24C+                                       |
...
| Runtime i18n NL/EN with teacher-style workflow        | FRONT-018                    | `vue-i18n` + Sheets exporter; 481 keys; es/zh Default caveat | done (23)    | Optional or extra in the original checklist; mandatory for this project’s chosen Tier A final scope |
...
| Submission docs freeze (checklists, Toggle, Leho ZIP) | DOC-007, DOC-008, SUBMIT-*   | README strong; final checklist attachments still due         | 33           | Not blocking Phase 24 coding                                                                        |
...
**Primary recommendation:** **B — mixed managed hosting** — **chosen in Phase 24A:** Firebase Hosting (PWA) + Railway API (exactly one replica, Serverless disabled) + MongoDB Atlas M0. See [`deployment.md`](./deployment.md).  
...
### Planned exam approach (Phase 23 — complete)

| Item            | Plan / outcome                                                                                        |
| --------------- | ----------------------------------------------------------------------------------------------------- |
| Locales         | **nl** (default), **en** (runtime fallback), **zh**, **es**                                           |
| Runtime         | `vue-i18n` Composition API + `useLanguage` + `CommonLanguageSelector` _(done)_                        |
| Source of truth | Google Sheet (editable) → exporter → committed locale JSON (runtime/CI); no Google in PWA             |
| Generated files | `packages/pwa/src/locales/{nl,en,zh,es}.json` — **481** identical keys                                |
| Exporter        | `packages/i18n-export` workspace (student-owned; Phase 23A)                                           |
| Persistence     | `localStorage['vaccin-delivery:locale']`                                                              |
| Missing keys    | Runtime → English catalog; export-time → Sheet Default (separate)                                     |
| Formatting      | BCP47 map via `format.ts` / `getLocaleBcp47`                                                          |
| Central helpers | status-labels, error-mapper, Zod validation-schema factories (locale-reactive)                        |
| Tests           | Exporter offline (23A); resolution/loader/`setLocale`; Playwright language-switch                     |
| Remaining       | **Done (23C)** — UI string migration complete; improve zh/es human translations in Sheet as follow-up |
| Quality caveat  | ~415 zh/es keys still Default (EN) export fallback — not verified human translations                  |
| Sync note       | Edit Sheet in Google UI, then `npm run export:i18n` (readonly; no offline merge)                      |
| offline.html    | Static multilingual; reads locale once from localStorage (not reactive)                               |
...
| CI             | 5 green workflows @ `f3e4d07` (Phases 0–20 baseline); do not invent separate 23C CI green claims               |
| Seed           | Idempotent CLI; gated; Firebase provisioning; demo accounts incl. `docent@howest.be`                           |
| i18n           | Phase 23 complete: Sheet → export → vue-i18n; FRONT-018 implemented with es/zh Default caveat                  |
...
_Document version: 2026-07-24 (Phase 23C complete)._
