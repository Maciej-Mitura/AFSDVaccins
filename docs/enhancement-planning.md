...
**Status:** Planning only (Phase 21). No application features implemented in this document.  
**Evidence date:** 2026-07-23 · branch `develop` @ `f3e4d07`  
...
| nginx                       | Strongly expected                                                                                       | `packages/pwa/nginx.conf`                        | Implemented           | Headers on PWA only                                                               | API headers Phase 22                             |
...
| Rate limiting               | Optional or extra in original checklist; mandatory for this project’s chosen Tier A final scope         | Not in deps / modules                            | Missing               | —                                                                                 | Phase 22 `@nestjs/throttler` (or equivalent)     |
| Response / memory caching   | Optional or extra in original checklist; mandatory for this project’s chosen Tier A final scope         | Not in API                                       | Missing               | SW caches shell only                                                              | Phase 22                                         |
...
| GraphQL complexity/depth    | Quality / security                                                                                      | Not found                                        | Missing               | —                                                                                 | Phase 22                                         |
| Helmet / CSP (API)          | CSRF/XSS checklist                                                                                      | nginx headers only                               | Partially implemented | API lacks Helmet                                                                  | Phase 22                                         |
...
| Backend rate limiting                                 | description extras           | No throttler                                                     | 22           | Optional or extra in the original checklist; mandatory for this project’s chosen Tier A final scope |
| Caching + invalidation                                | description extras           | No API cache                                                     | 22           | Optional or extra in the original checklist; mandatory for this project’s chosen Tier A final scope |
| CSRF/XSS threat-model documentation + headers         | API-012                      | Partial (CORS + validation + nginx); no API Helmet/CSP narrative | 22           | Must document SPA+Bearer model; add headers                                                         |
| Submission docs freeze (checklists, Toggle, Leho ZIP) | DOC-007, DOC-008, SUBMIT-*   | README strong; final checklist attachments still due             | 33           | Not blocking Phase 22 coding                                                                        |
...
| ----------------------------------------------- | ---------------- | --------------------------------------------- | ------------------------------ |
| GraphQL query complexity / depth limits         | API-012 adjacent | Absent                                        | 22                             |
...
| Documented CSRF/XSS engineering controls        | API-012          | Partial                                       | 22 + docs in 33                |
