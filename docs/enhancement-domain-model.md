...
| **Lifecycle**       | Generate+mint with route → authorised SVG retrieval / safe metadata (26B) → courier scan preview does not consume (26C) → confirm consume (26D). Clearing `encodedToken` after consume may wait until 26D                                                                                                      |
| **Ownership**       | **Preview (26C):** assigned BEZORGER only while route `IN_PROGRESS`. **Confirm (26D):** assigned BEZORGER; ADMIN oversight. **Retrieval (26B):** ADMIN or recipient APOTHEKER (`DeliveryStop.apothekerUserId`) while route `ASSIGNED`/`IN_PROGRESS` and QR unconsumed                                          |
