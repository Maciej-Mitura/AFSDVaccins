...
| **Lifecycle**       | Generate+mint with route → encode QR image / narrow retrieval (26B) → scan preview does not consume → confirm consume (26C). Clearing `encodedToken` after consume may wait until 26D                                                                                                                          |
| **Ownership**       | Assigned BEZORGER confirms while route `IN_PROGRESS`; ADMIN oversight                                                                                                                                                                                                                                          |
| **Retention**       | Long-lived on the generated route document                                                                                                                                                                                                                                                                     |
| **Sensitivity**     | High bearer (`encodedToken`); never GraphQL on ordinary route/stop; redact from PubSub/audit/logs; safe fields only (`qrAvailable`, `qrConsumed`, `deliveredAt`)                                                                                                                                               |
| **Storage**         | **Embedded on `DeliveryRoute.stops[]`** (not on `RouteTemplate`). A separate collection remains optional later for cross-route audit export                                                                                                                                                                    |
| **Legacy**          | Pre-26A routes without QR state remain readable; no startup backfill (policy C)                                                                                                                                                                                                                                |
| **Phase**           | 26A foundation; 26B/26C + roadmap 27 for issue/scan/confirm/manifest                                                                                                                                                                                                                                           |
