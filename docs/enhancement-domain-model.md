...
| **Lifecycle**       | Generate+mint with route → authorised SVG retrieval / safe metadata (26B) → scan preview does not consume → confirm consume (26C). Clearing `encodedToken` after consume may wait until 26D                                                                                                                    |
| **Ownership**       | Assigned BEZORGER confirms while route `IN_PROGRESS` (26C); ADMIN oversight. **Retrieval (26B):** ADMIN or recipient APOTHEKER (`DeliveryStop.apothekerUserId`) while route `ASSIGNED`/`IN_PROGRESS` and QR unconsumed                                                                                         |
...
| **Storage**         | **Embedded on `DeliveryRoute.stops[]`** (not on `RouteTemplate`). Rendered QR images are **never** persisted (on-demand SVG only). A separate collection remains optional later for cross-route audit export                                                                                                   |
...
| **Phase**           | 26A foundation; 26B retrieval/SVG; 26C/26D + roadmap 27 for scan/confirm/manifest                                                                                                                                                                                                                              |
