/**
 * Sync Phase 30B coarse-location UI keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase30b-keys.ts
 */
...
import { PHASE_30B_I18N_KEYS } from './phase30b-keys.js'
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_30B_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 30B keys (preview)…`
      : `Syncing ${specs.length} Phase 30B keys…`,
  )
  ...
  console.info('Phase 30B key sync complete. Run npm run export:i18n next.')
}
