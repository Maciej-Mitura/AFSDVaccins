/**
 * Sync Phase 34C route voice-report UI keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase34c-keys.ts
 */
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_34C_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 34C keys (preview)…`
      : `Syncing ${specs.length} Phase 34C keys…`,
  )
  ...
  console.info('Phase 34C key sync complete. Run npm run export:i18n next.')
}
