/**
 * Sync Phase 32B courier-analytics UI keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase32b-keys.ts
 */
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_32B_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 32B keys (preview)…`
      : `Syncing ${specs.length} Phase 32B keys…`,
  )
  ...
  console.info('Phase 32B key sync complete. Run npm run export:i18n next.')
}
