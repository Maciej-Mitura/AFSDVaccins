/**
 * Sync Phase 25F vaccine-image keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase25f-keys.ts
 */
...
import { PHASE_25F_I18N_KEYS } from './phase25f-keys.js'
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_25F_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 25F keys (preview)…`
      : `Syncing ${specs.length} Phase 25F keys…`,
  )
  ...
  console.info('Phase 25F key sync complete. Run npm run export:i18n next.')
}
