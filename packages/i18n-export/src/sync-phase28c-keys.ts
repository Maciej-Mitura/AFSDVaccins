/**
 * Sync Phase 28C arrival keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase28c-keys.ts
 */
...
import { PHASE_28C_I18N_KEYS } from './phase28c-keys.js'
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_28C_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 28C keys (preview)…`
      : `Syncing ${specs.length} Phase 28C keys…`,
  )
  ...
  console.info('Phase 28C key sync complete. Run npm run export:i18n next.')
}
