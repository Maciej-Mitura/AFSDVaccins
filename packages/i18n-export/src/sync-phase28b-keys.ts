/**
 * Sync Phase 28B offline viewing keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase28b-keys.ts
 */
...
import { PHASE_28B_I18N_KEYS } from './phase28b-keys.js'
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_28B_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 28B keys (preview)…`
      : `Syncing ${specs.length} Phase 28B keys…`,
  )
  ...
  console.info('Phase 28B key sync complete. Run npm run export:i18n next.')
}
