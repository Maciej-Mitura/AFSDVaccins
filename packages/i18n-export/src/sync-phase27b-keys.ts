/**
 * Sync Phase 27B notification UX keys to Google Sheets, then export locale JSON.
 * Run: npm exec --workspace=@vaccin-delivery/i18n-export -- tsx src/sync-phase27b-keys.ts
 */
...
import { PHASE_27B_I18N_KEYS } from './phase27b-keys.js'
...
async function main(): Promise<void> {
  ...
  const specs: KeySyncSpec[] = PHASE_27B_I18N_KEYS.map(entry => ({
    key: entry.key,
  ...
  console.info(
    dryRun
      ? `Planning ${specs.length} Phase 27B keys (preview)…`
      : `Syncing ${specs.length} Phase 27B keys…`,
  )
  ...
  console.info('Phase 27B key sync complete. Run npm run export:i18n next.')
}
